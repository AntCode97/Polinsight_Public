const isValidPassword = (passwordValue) => {
  return /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{10,16}$/.test(
    passwordValue
  );
};

const isValidEmail = (emailValue) => {
  return /^[a-zA-Z0-9\.\_\-]{6,20}@([a-zA-Z0-9])+\.[a-zA-Z0-9]{2,8}$/i.test(
    emailValue
  );
};
...
const isValidPhone = (phoneValue) => {
  return (
    /^[0-9]{3}[0-9]{4}[0-9]{4}$/.test(phoneValue) ||
    /^[0-9]{3}-[0-9]{4}-[0-9]{4}$/.test(phoneValue)
  );
};
